...
import com.daveleron.jobber.swipeview.CardStackAdapter
import com.daveleron.jobber.swipeview.StackDiffCallback

class SwipeScreenActivity : AppCompatActivity(), CardStackListener {

    private val dataStore : DataStorage = DataStorage.instance
    private val drawerLayout by lazy { findViewById<DrawerLayout>(R.id.drawer_layout) }
    private val cardStackView by lazy { findViewById<CardStackView>(R.id.card_stack_view) }
    private val manager by lazy { CardStackLayoutManager(this, this) }
    private val adapter by lazy { CardStackAdapter(dataStore.startupStorageList) }

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_swipescreen)
    ...
    override fun onCardSwiped(direction: Direction) {
        ...
        if (direction.toString().equals("Right")){
            DataStorage.instance.selectedList.add(adapter.getStartup().get(manager.topPosition - 1))
            println("SIZE OF LIST: " + DataStorage.instance.selectedList.size)
        }
...
//    }
    ...
    private fun paginate() {
        val old = adapter.getStartup()
        val new = old.plus(dataStore.startupStorageList)
        val callback = StackDiffCallback(old, new)
        val result = DiffUtil.calculateDiff(callback)
        adapter.setStartup(new)
        result.dispatchUpdatesTo(adapter)
    }

    private fun reload() {
        val old = adapter.getStartup()
        val new = dataStore.startupStorageList
        val callback = StackDiffCallback(old, new)
        val result = DiffUtil.calculateDiff(callback)
        adapter.setStartup(new)
        result.dispatchUpdatesTo(adapter)
    ...
    private fun addFirst(size: Int) {
        val old = adapter.getStartup()
        val new = mutableListOf<Startup>().apply {
            addAll(old)
    ...
    private fun createSpot(): Startup {
        return Startup(
            "Gogle","gogl@test.com","gogl.com",
            "1234 AB", "United-States", "English", R.drawable.gogle)
    }

    public fun onButtonChatsClick(view: View){
        val intent = Intent(this@SwipeScreenActivity, ChatScreenActivity::class.java)
